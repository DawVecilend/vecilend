        ...
        <div className="flex flex-col gap-2">
          {categories.map((cat) => (
            <div key={cat.id} className="rounded-xl border border-app-border overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-4 bg-app-bg-card">
                <button
                  onClick={() => setExpanded((p) => ({ ...p, [cat.id]: !p[cat.id] }))}
                  className="text-app-text-secondary w-4"
                >
                  {expanded[cat.id] ? "▾" : "▸"}
                </button>
                ...
                <span className="text-xs text-app-text-secondary hidden sm:block">
                  {cat.subcategories_count ?? cat.subcategories?.length ?? 0} subcats.
                </span>
                ...
                {editingId === cat.id ? (
                  <>
                    ...
                    <button
                      onClick={cancelEdit}
                      className="text-xs px-3 py-1 rounded-lg bg-app-neutral text-app-text-secondary"
                    >
                      Cancelar
                    </button>
                  </>
              ...
              {expanded[cat.id] && (
                <div className="border-t border-app-border bg-app-bg">
                  {cat.subcategories?.length > 0 ? cat.subcategories.map((sub) => (
                    <div key={sub.id} className="flex items-center gap-3 px-5 py-3 pl-12 border-b border-app-border">
                      <span className="w-1.5 h-1.5 rounded-full bg-app-primary flex-shrink-0" />
