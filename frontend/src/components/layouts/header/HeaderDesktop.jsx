      ...
      <div className="flex items-center gap-4">
        {user ? (
          <>
            ...
            <PrimaryButton url="/chats" text="Chats" width="130px" />

            <div onClick={() => setOpen(!open)} className='cursor-pointer'>
              {user.avatar_url ? (
                <img src={user.avatar_url} alt="Avatar usuario" className="h-[48px] w-[48px] rounded-full object-cover" />
              ) : (
                <img src="/assets/icons/empty-user-icon.svg" alt="Usuario sin foto" className="h-[48px]" />
              )}
            </div>
            <div ref={menuRef} className="relative">
              {open && (
                <div className="absolute top-10 right-2 z-20 w-56">
                  <div className="bg-vecilend-dark-bg-secondary shadow-lg border border-white/5 overflow-hidden backdrop-blur-sm">
                    <div className="flex flex-col divide-y divide-[#14B8A6]/10">
                      <Link to="/" className="px-4 py-3 text-sm text-[#14B8A6] hover:bg-white/5 hover:text-white transition-colors">
